...
This is a template for browser extension projects. Please read the `README.md` file at https://github.com/ericchase-library/ts-library for more information.
...
- This project produces a browser extension (addon) for Chrome and Firefox.
...
This folder contains a subfolder for each browser targeted in the `./src/manifest.json` file (Chrome and Firefox by default).

- A `.zip` archive is built for each version of the extension under each subfolder. After properly configuring the manifest file, the resulting archives should be ready for publishing.
- You can also find a `temp` folder under each subfolder that may be loaded as a temporary extension for testing and debugging purposes.
